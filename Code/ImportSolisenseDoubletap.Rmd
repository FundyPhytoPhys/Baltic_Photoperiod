...
```{r set project variables for data taken from old software; OS}
Project <- "Baltic_Photoperiod"
DataOut <- file.path("..", "Data", "ImportedData")
DataIn <- file.path("..", "Data", "RawData", "SolisenseTest", fsep = .Platform$file.sep)
...
SolisenseFilesOS <- list.files(path = DataIn, pattern = FileID, full.names = TRUE, recursive = FALSE)
SolisenseFilesOS

#test for duplicate file names
unique(duplicated(SolisenseFilesOS))
```
...
SolFitsTrimOS <- SolFitsOS %>%
  ...
  mutate(Filename = str_remove(string = Filename, pattern = "/RawData/SolisenseTest/"),
         Filename = str_remove(string = Filename, pattern = "_fit.csv")
         ) %>%
  separate(Filename, into = c("fp0", "Project", "RunDateTime", "CultureID", "Ex_WL"), sep = "([\\/\\_])", remove = FALSE) %>%
  ...
  select(-c(Light_1,Light_2,Light_3,Light_4,Light_5,Light_6,`Source DataFile`,DATE,TIME,RunDateTime, fp0, Project))
...
```{r test plot}
SolFitsTrimMetaDarkafterLightOS %>% 
  ggplot() +
...
```{r set project variables for data taken from new software; NS}
Project <- "Baltic_Photoperiod"
DataOut <- file.path("..", "Data", "ImportedData")
DataIn <- file.path("..", "Data", "RawData", "SolisenseNewComp", fsep = .Platform$file.sep)
...
```{r list files for file import}
SolisenseFilesNS <- list.files(path = DataIn, pattern = FileID, full.names = TRUE, recursive = FALSE)
SolisenseFilesNS

#test for duplicate file names
unique(duplicated(SolisenseFilesNS))
```
...
head(SolFitsNS)
colnames(SolFitsNS)
length(unique(SolFitsNS$Filename))
```
...
SolFitsTrimNS <- SolFitsNS %>%
  ...
  mutate(Filename = str_remove(string = Filename, pattern = "/RawData/SolisenseNewComp/"),
         Filename = str_remove(string = Filename, pattern = "_fit.csv")
         ) %>%
  separate(Filename, into = c("fp0", "Project", "RunDateTime", "CultureID", "Ex_WL"), sep = "([\\/\\_])", remove = FALSE) %>%
  mutate(RunDateTime = ymd_hm(RunDateTime),
...
```{r test plot}
SolFitsTrimMetaDarkafterLightNS %>% 
  ggplot() +
  geom_point(aes(x = PARPhotonDose, y = meanSig1s_nm2psii, colour = as.factor(Par_ue), shape = as.factor(Photoperiod)), size = 3.5, show.legend = T) +
  ggh4x::facet_nested(cols = vars(Ex_WL), rows = vars(Strain), labeller = label_parsed) +
  theme_bw() 
...
```{r save rds}
saveRDS(SolFitsMeta, file.path(DataOut, paste(Project, "Imported_SolisenseLight.Rds", sep = "_"), fsep = .Platform$file.sep))

saveRDS(SolFitsMetaDarkafterLight, file.path(DataOut, paste(Project, "Imported_SolisenseDarkafterLight.Rds", sep = "_"), fsep = .Platform$file.sep))
```
