---
title: "Process OLIS spectral data and Jaz data; create final pigments plot"
...
PIGMENTS - for now - taken from different repository
...
MCPigmentFile <- "../Data/ProcessedData/Baltic_Photoperiod_Pigments_OtherRepository.Rds"
...
MCPigment <- MCPigment                     # Duplicate data frame
MCPigment[MCPigment < 0] <- 0     # Set negative values to 0
...
#   ggh4x::facet_nested(rows = vars(Strain), cols = vars(PigmentsName), labeller = label_parsed) +
#   theme_bw()
```
...
```{r Test plot - pigment ratio, fig.height = 8, fig.width = 8}
# lab1=c(expression("PC-rich_056"), expression("PC-rich_077"), expression("PE-rich_048"), expression("PE-rich_127"))
# lab2=c(expression("8 h"), expression("12 h"), expression("16 h"), expression("24 h"))
# lab3=c(expression("30 µE"), expression("90 µE"), expression("180 µE"), expression("300 µE"), expression("900 µE"))
# 
...
#   scale_shape_manual(values = c(15, 16, 17, 18), name="", labels = lab2) +
#   scale_colour_manual(values = c("darkslategray", "lightblue4", "hotpink4", "indianred3", "orange1"), name="", labels = lab3) +
#   ggh4x::facet_nested(rows = vars(Strain), cols = vars(PigmentsRatioName), labeller = label_parsed) +
#   theme_bw() 
# 
# MCPigmentRatioPlot %>%
#   ggplot() +
...
MCPigmentRatioPlot %>%
  ...
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
  labs(y = "Pigments ratio", x = "Elapsed time (h)") +
  ggh4x::facet_nested(cols = vars(facetsPARPhotonDose, PARPhotonDoseTidy), rows = vars(factor(PigmentsRatioName, levels=c('Total~Phyco/Chl~a~ratio', 'Car/Chl~a~ratio'))), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        axis.text = element_text(size=12),
