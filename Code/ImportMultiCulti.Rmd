...
```{css, Set figure caption font size, echo=FALSE}
p.caption {
  font-size: 18px;
}
```
...
```{r List previously imported files}
list.files(path = DataOut, pattern = Project, full.names = TRUE)
```

```{r List files in the extracted folder with a ".csv" extension}
