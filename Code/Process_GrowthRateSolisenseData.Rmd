...
GrowthRateSoliExp <- GrowthRate %>% 
  left_join(., Solisense, by = c("SampleID"="SampleID", "Run"="Run",  "Strain"="Strain", "ExpDate"="ExpDate", "Par_ue"="Par_ue","Photoperiod"="Photoperiod", "O2"="O2", "WL"="WL", "PARPhotonDose_day"="PARPhotonDose_day")) 
```
...
SolFitsAllPigmentTurnerDayfit <- SolFitsAllPigmentTurnerDay %>%  
  select(c(Strain, deltaOD_Lmu_corr, JVPSII_ETRqpOxbo_FoSig_Corrcell_day, Par_ue, Photoperiod))  %>% 
  mutate(JVPSII_ETRqpOxbo_FoSig_Corrcell_day=JVPSII_ETRqpOxbo_FoSig_Corrcell_day/10000000000000000000)
  
SolFitsAllPigmentTurnerDayfittest <- SolFitsAllPigmentTurnerDayfit %>%
  nest(data = -c("Strain")) %>%  
  mutate(GRCplatt_Model = map(data, ~nlsLM(deltaOD_Lmu_corr ~ grcplatt(I = JVPSII_ETRqpOxbo_FoSig_Corrcell_day, a, b, Pmax),
                                                    data = .x,
                                                    start = list(a = 2/1, b = 2/4, Pmax = 5),
                                                    lower = c(0, 0, 0)))) %>%    
...
GRCplatt_PredictGrowth <- SolFitsAllPigmentTurnerDayfit %>% 
...
# scientific_10 <- function(x) {   parse(text=gsub("e\\+*", " %*% 10^", scales::scientific_format()(x))) }
# 
# ggplot(SolFitsAllPigmentTurnerDay, aes(JVPSII_ETRqpOxbo_FoSig_Corrcell_day, deltaOD_Lmu_corr)) +
#   geom_point(aes(x=JVPSII_ETRqpOxbo_FoSig_Corrcell_day, y=deltaOD_Lmu_corr), alpha = 0.9, size = 3.5, show.legend = T) +
#   # geom_errorbar(aes(x = PhotonDose_value, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se, colour = as.factor(Par_ue)), width=4000000, data = . %>% filter(deltaOD_Lmu_se<8.057558e-03), show.legend = F) +
#   geom_line(aes(JVPSII_ETRqpOxbo_FoSig_Corrcell_day, .fitted), show.legend = F, GRCplatt_PredictGrowth) +
#   scale_x_continuous(label=scientific_10) +
#   scale_linetype_manual(values=c("equal900" = "dotted", "less900" = "solid"), breaks=c('8', '12', '16', "24", "", "")) +
#   scale_colour_manual(values = c("", "", "darkslategray", "lightblue4", "hotpink4", "indianred3", "orange1"), name="", labels = lab2, breaks=c('', '', '30', "90", "180", "300", "900")) +
#   scale_shape_manual(values = c(15, 16, 17, 18), name="", labels = lab1) +
