...
data_text_y0<- data.frame(facetsPhase = c("Phase~of~growth", "Phase~of~growth"), Phase = c("Exponential"), facetsStrain=c("Strain", "Strain", "Strain", "Strain"), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"),  label = c('y0 = 2.6', 'y0 = 1.6', 'y0 = 1.2', 'y0 = 1.4'))

data_text_yf<- data.frame(facetsPhase = c("Phase~of~growth", "Phase~of~growth"), Phase = c("Exponential"), facetsStrain=c("Strain", "Strain", "Strain", "Strain"), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"),  label = c('yf = 0.8', 'yf = 0.5', 'yf = 0.3', 'yf = 0.5'))

data_text_lambda<- data.frame(facetsPhase = c("Phase~of~growth", "Phase~of~growth"), Phase = c("Exponential"), facetsStrain=c("Strain", "Strain", "Strain", "Strain"), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"),  label = c('\u03BB = 3.4', '\u03BB = 1.7', '\u03BB = 1.2', '\u03BB = 2.1'))

data_segment<- data.frame(x=1000000,y=0,xend=26000000,yend=0, facetsPhase = c("Phase~of~growth"), Phase = c("Pre-stationary", "Pre-stationary", "Pre-stationary", "Pre-stationary"), facetsStrain=c("Strain", "Strain", "Strain", "Strain"), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"))
...
lab2=c(expression("30 µE"), expression("90 µE"), expression("180 µE"), expression("300 µE"), expression("900 µE"))
  ...
  geom_point(aes(x = PARPhotonDose_day, y = GS, colour = as.factor(Par_ue), shape = as.factor(Photoperiod)), size = 1, alpha = 0.8, show.legend = F, GScatalogMetaFinalPlot) +
  geom_line(aes(PARPhotonDose_day, .fitted), show.legend = F, GSMetaExpPlot) +
  geom_hline(yintercept = 1, linetype="dashed", size=0.4) +
  geom_text(data=data_text_y0, aes(x=24000000, y=3, label=label), size=3.5) +
  geom_text(data=data_text_yf, aes(x=24000000, y=2.7, label=label), size=3.5) +
  geom_text(data=data_text_lambda, aes(x=24000000, y=2.4, label=label), size=3.5) +
  #geom_segment(data=data_segment, aes(x=x,y=y,yend=yend,xend=xend),inherit.aes=FALSE)+
  labs(y = "Diel growth symmetry (AccLen/DecLen ratio)", x = "PAR photon dose ( µmol photons "~m^-2~""~d^-1~")") +
...
GScatalogMeta %>%
  ...
  geom_line(aes(x = PARPhotonDose_day, y = meanAccLen, linetype=as.factor(Photoperiod)), size = 0.3, show.legend = F) +
  ...
  geom_text(data=data_text_8, aes(x=36000000, y=3.5, label=label), size=2.5) +
  geom_text(data=data_text_12, aes(x=36000000, y=5.5, label=label), size=2.5) +
  geom_text(data=data_text_16, aes(x=36000000, y=7.5, label=label), size=2.5) +
  
  geom_hline(data = data_hlineGS8, aes(yintercept = value), linetype="dotted", size=0.4) +
  geom_hline(data = data_hlineGS12, aes(yintercept = value), linetype="dotted", size=0.4) +
  geom_hline(data = data_hlineGS16, aes(yintercept = value), linetype="dotted", size=0.4) +
  ...
  coord_cartesian(ylim = c(0, 12), xlim = c(0, 40000000)) +
  ggh4x::facet_nested(cols = vars(facetsPhase, Phase), rows = vars(facetsStrain, Strain), labeller = label_parsed) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
...
rm(data_hlineGS8, data_hlineGS12, data_hlineGS16, GScatalog, GScatalogMetaExp,  CultureCatalog, tMaxAGLines_056, tMaxAGLines_077, tMaxAGLines_048, tMaxAGLines_127, GScatalogMetaFinalPlot, GSMetaExp, GSMetaExpPlot)
```
