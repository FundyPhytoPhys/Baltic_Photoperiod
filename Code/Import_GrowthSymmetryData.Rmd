---
title: "Import_GrowthSymmetryData"
author:
...
GScatalogMeta<-GScatalogMeta %>% 
  ...
  summarize(Run, CultureID, Strain, ExpDate, Par_ue, Photoperiod, PARPhotonDose, MC, Tube, O2, LightShape, ExpEndDate, PhotoperiodTstart_h, PhotoperiodTend_h,tMaxDG,tMaxAG, Phase, AchivePreStationary, MaxDG, MaxAG, Acc_perc, Dec_perc, Strain, Par_ue, Photoperiod, O2, WL, OD680start, OD680end, TDG, AccDecRatio_perc, AccLen, DecLen, GS, 
...
# Calculate Par_ue Sine and PARphotondose Sine (because of sine shape of light)

```{r change as character to extract first digit without round - b/c this is hours}
GScatalogSine<-GScatalogMeta %>% 
  mutate(meanAccLen = as.character(meanAccLen)) 

GScatalogSine$meanAccLenRound<- substr(GScatalogSine$meanAccLen, 1, 1)

GScatalogSine<-GScatalogSine %>% 
  mutate(meanAccLen = as.numeric(meanAccLen)) %>% 
  mutate(meanAccLenRound = as.numeric(meanAccLenRound)) 
```

# Read Sine Par_ue catalog

```{r load df with calculated Par_ue sine per hour, results = "hide", warning = FALSE, echo=FALSE}
gs4_deauth()
# this is the URL or ID of a Sheet readable by anyone (with a link)
...
GScatalogSineMeta <- GScatalogSine %>%
...
GScatalogSineMeta %>%
  ggplot() +
  geom_point(aes(x = PARPhotonDoseSine, y = meanGS, colour = as.factor(Par_ue), shape = as.factor(Photoperiod)), size = 3.5, alpha = 0.8, show.legend = T) +
  geom_errorbar(aes(x = PARPhotonDoseSine, ymin = meanGS - sdGS, ymax = meanGS + sdGS, colour = as.factor(Par_ue)), show.legend = F, width=0) +
  geom_hline(yintercept = 1, linetype="dashed") +
  #labs(y = "Diel growth symmetry (AccLen/DecLen)", x = "PAR photon dose ( µmol photons "~m^-2~""~d^-1~")") +
  scale_shape_manual(values = c(15, 16, 17), name="", labels = lab1) +
  scale_colour_manual(values = c("darkslategray", "lightblue4", "hotpink4", "indianred3", "orange1"), name="", labels = lab2) +
...
rm(data_hlineGS8, data_hlineGS12, data_hlineGS16, GSMetaSt, augmented, fitted, GScatalog, GScatalogMetaExp, GSMetaExp, CultureCatalog, data_vline_048, data_vline_127, data_vline_056, data_vline_077, ParSine, GSMetaExpPlot, GScatalogSine, GScatalogMetaFinalPlot, GScatalogSineMeta)
```

# Save rds for further analysis

```{r save rds}
saveRDS(GScatalogMeta, file.path(DataOut, paste(Project, "Imported_GrowthSymmetryData.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
```
