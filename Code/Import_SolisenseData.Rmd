...
# Choose the same variables to merge two df

```{r}
SolFitsTrimMetaCleanOS<-SolFitsTrimMetaCleanOS %>% 
  select(-c(ChiSq, "Fv/Fo", p, fmPQ, PQP_Size, carQ, carQt, p680Q, p680Qt, fQB))

SolFitsTrimMetaDarkafterLightOS<-SolFitsTrimMetaDarkafterLightOS %>% 
  select(-c(ChiSq, "Fv/Fo", p, fmPQ, PQP_Size, carQ, carQt, p680Q, p680Qt, fQB))

SolFitsTrimMetaCleanNS<-SolFitsTrimMetaCleanNS %>% 
  select(-c("Dur-Wat", ChiSq, QARredMax, p, Alp4QA, Tau4QA))
