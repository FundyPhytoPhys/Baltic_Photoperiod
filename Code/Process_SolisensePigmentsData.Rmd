...
Sol11<-SolFitsExpSt %>% 
  filter(Phase == "Pre-stationary") %>% 
  filter(Par_ue ==900) %>% 
  filter(Strain=="PC-rich_077" | Strain=="PE-rich_048" | Strain=="PE-rich_127") %>% 
  filter(Sig<300)
...
SolFitsExpFitAll<-rbind(Sol1, Sol2, Sol3, Sol4, Sol5, Sol6, Sol7, Sol10, Sol11, Sol12)

rm(Sol1, Sol2, Sol3, Sol4, Sol5, Sol6, Sol7, Sol10, Sol11, Sol12)
```
...
SolFitsExpFit590Clean <-SolFitsExpFitAll %>% 
  ...
  #filter(Strain == "PE-rich_127") %>% 
  filter(Ex_WL == 590) %>% 
  select(c(Sig_nm2psii, PARPhotonDose_day, Strain, Phase)) 
...
SolFitsExpSt590Plot <- SolFitsExpFit590 %>%
  left_join(., augmented, by = c("Strain"="Strain", "PARPhotonDose_day"="PARPhotonDose_day", "Sig_nm2psii"="Sig_nm2psii"))

SolFitsExpSt445 <-SolFitsExpFitAll %>%
  filter(Ex_WL == 445) %>%
  mutate(.fitted=NA)

SolFitsExpStPlot<-rbind(SolFitsExpSt590Plot, SolFitsExpSt445)
```
...
data_text_y0<- data.frame(WL = c("WW", "WW"), Phase = c("Exponential"), O2=c(21, 21, 21, 21), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"),  label = c('y0 = 7.2', 'y0 = 5.8', 'y0 = 8.5', 'y0 = 7.9'))

data_text_yf<- data.frame(WL = c("WW", "WW"), Phase = c("Exponential"), O2=c(21, 21, 21, 21), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"),  label = c('yf = 3.2', 'yf = 1.5', 'yf = 3.8', 'yf = 4.1'))

data_text_lambda<- data.frame(WL = c("WW", "WW"), Phase = c("Exponential"), O2=c(21, 21, 21, 21), Strain  = c("PC-rich_056", "PC-rich_077", "PE-rich_048", "PE-rich_127"),  label = c('\u03BB = 2.8', '\u03BB = 2.6', '\u03BB = 1.9', '\u03BB = 3.6'))
...
ggplot(SolFitsExpFitAll, aes(x = PARPhotonDose_day, y = Sig_nm2psii)) +
  ...
  #geom_line(aes(PARPhotonDose_day, .fitted), show.legend = F, SolFitsExpStPlot) +
  ...
  geom_text(data=data_text_lambda, aes(x=80000000, y=6.2, label=label), size=3.5) +
  scale_shape_manual(values = c(15, 16, 17,18), name="", labels = lab1) +
  scale_colour_manual(values = c("darkslategray", "lightblue4", "hotpink4", "indianred3", "orange1"), name="", labels = lab2) +
...
saveRDS(SolFitsExpStPlot, file.path(RDSPlotPath, paste(Project, "Plot_Sigma_2.Rds", sep = "_"), fsep = .Platform$file.sep), ascii = FALSE, version = NULL, compress = "xz", refhook = NULL)
