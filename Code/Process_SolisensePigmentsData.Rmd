...
SolFitsAllPigmentPCPEMerge1<-SolFitsAllPigmentPCPE %>% 
  select(c(Strain, Ex_WL, WL, Par_ue, Photoperiod, Sig_nm2psii, PhycoChlaRatioExp)) %>% 
  filter(Ex_WL ==590) %>% 
  mutate(DifferentSigma = WL) %>%
  mutate(DifferentSigma=case_when(DifferentSigma=="WW"~"SigmaLight")) %>% 
  rename(Sigma_value=Sig_nm2psii)

SolFitsAllPigmentPCPEMerge2<-SolFitsAllPigmentPCPE %>% 
  select(c(Strain, Ex_WL, WL, Par_ue, Photoperiod, Sigdark_nm2psii, PhycoChlaRatioExp)) %>% 
  filter(Ex_WL ==590) %>% 
  mutate(DifferentSigma = WL) %>%
  mutate(DifferentSigma=case_when(DifferentSigma=="WW"~"SigmaDark")) %>% 
  rename(Sigma_value=Sigdark_nm2psii)

SolFitsAllPigmentPCPEMerge<-rbind(SolFitsAllPigmentPCPEMerge1, SolFitsAllPigmentPCPEMerge2)



SolFitsAllPigmentfitPhycoChlalinearMerge <- SolFitsAllPigmentPCPEMerge %>%   
  select(c(Strain, Par_ue, Photoperiod, Ex_WL, Sigma_value, PhycoChlaRatioExp, WL, DifferentSigma)) %>%  
  nest(data = -c("Par_ue", "DifferentSigma")) %>%   
...
SolFitsAllPigmentfitPhycoChlalinearMerge %>% 
  unnest(Linear_Model_Predict) %>% 
  ggplot() +
  geom_point(aes(x = PhycoChlaRatioExp, y = Sigma_value, colour=as.factor(Strain)), size = 3.5, show.legend = T, SolFitsAllPigmentPCPEMerge) + 
  # geom_errorbar(aes(x = PhycoChlaRatioExp, ymin = meanSig_nm2psii - sdSig_nm2psii, ymax = meanSig_nm2psii + sdSig_nm2psii, colour=as.factor(Strain)), SolFitsAllPigmentPCPE) +
  geom_line(aes(x = PhycoChlaRatioExp, y = `.fitted`), show.legend = F) +   
  # scale_shape_manual(values = c(15, 16, 17,18), name="", labels = lab1) +
  # scale_colour_manual(values = c("darkslategray", "lightblue4", "hotpink4", "indianred3", "orange1"), name="", labels = lab2) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
  ggh4x::facet_nested(cols = vars(DifferentSigma), rows = vars(Strain), labeller = label_parsed) +
  theme_bw() 
```
