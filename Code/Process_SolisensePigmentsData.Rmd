...
```{r}
SolFitsExp<-SolFitsExp %>% 
  group_by(Strain, Ex_WL, Par_ue, Photoperiod) %>%
  summarize(c(SampleID, Run, Strain, ExpDate, Par_ue, Photoperiod, MC, Tube, O2, WL, LightShape, ExpEndDate, PARPhotonDose_day, Filename, ObsDate, ObsTime, Ex_WL, Sig, Sig_nm2psii, Sigdark, Sigdark_nm2psii, ActPARCorr_photonsm2s, FoOxbo, qpOxbo,  JVPSII_aLHIIOxbomax, JVPSII_ETRtauav_FoSig, JVPSII_ETRqpOxbo_FoSig, JVPSII_ETRtauav_aLHII_Sig, JVPSII_ETRqpOxbo_aLHII_Sig, E_days, Time_h, facetsPhotonDose_day, facetsExcitation, facetsStrain,
...
SolFitsExp1s %>% # data from second df contained sig 1s
  filter(Par_ue !=600) %>% 
  ggplot() +
  geom_point(aes(x = PARPhotonDose_day, y = meanSig1s_nm2psii, colour = as.factor(Par_ue), shape = as.factor(Photoperiod)), size = 3.5, show.legend = T) +
