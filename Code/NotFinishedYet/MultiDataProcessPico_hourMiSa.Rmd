...
```{r read ProcessFile}
ProcessFile <- "../ImportedData/20230816_PICO_MC257_RUN121_TargetDataMetaFilter.Rds"
...
ProcessData <- ProcessData %>%
  ...
  select(-c("Plate", "Well", "MovMAD680", "MovMAD720",  "MovMedian680", "MovMedian720", "IsMovMAD680Outlier", "IsMovMAD720Outlier","ActinicMin_parOD680", "ActinicMid_parOD680", "ActinicMin_parOD720", "ActinicMid_intervalOD720", "abs_time" ))

colnames(ProcessData)

#Need to add other strains (Sylwia's)- Ask Doug if this is best way to do this or do we comment this when running other strains?
...
ProcessDataNestGrowth  <- ProcessData2 %>%
...
ProcessDataNestGrowth <- ProcessDataNestGrowth %>%
  ...
  mutate(lognormOD720_GompLag = map_dbl(.x = lognormOD720_Gompertz_tidied, possibly(~ as.numeric(.[[2]][[1]]), otherwise = NA))
         ) %>%    #could possibly be replaced with map_depth to generate GLagNeg column without extracting GompLag column     
  mutate(IsOD720_GLagNeg =  if_else(lognormOD720_GompLag < 0, 1, 0)
         ) %>%
  mutate(lognormOD720_TimetoMax = map(.x = lognormOD720_Gompertz_predict, possibly(~.$time[which.max(.$`.fitted`)], otherwise = NULL))  
         ) %>%
  mutate(lognormOD720_TimetoMax = as.numeric(unlist(lognormOD720_TimetoMax))
         ) %>%
  ...
  mutate(lognormdeltaOD_GompLag = map_dbl(.x = lognormdeltaOD_Gompertz_tidied, possibly(~ as.numeric(.[[2]][[1]]), otherwise = NA))
         ) %>%         
  mutate(IsdeltaOD_GLagNeg =  if_else(lognormdeltaOD_GompLag < 0, 1, 0)
         ) %>%
  mutate(lognormdeltaOD_TimetoMax = map(.x = lognormdeltaOD_Gompertz_predict, possibly(~.$time[which.max(.$`.fitted`)], otherwise = NULL))  
         ) %>%
  mutate(lognormdeltaOD_TimetoMax = as.numeric(unlist(lognormdeltaOD_TimetoMax))
         ) %>%
  ...
  mutate(OD720_TimetoMax = as.numeric(unlist(OD720_TimetoMax))
         ) %>%
  ...
  mutate(OD720trunc_TimetoMax = as.numeric(unlist(OD720trunc_TimetoMax))
         ) %>%
   mutate(OD720trunc_exp = map(tubedata, possibly(~nlsLM(OD720 ~ exp_eqn(x = time, Emu, Eintercept),
         ...
         OD720trunc_exp_predict = map(OD720trunc_exp, possibly(augment, otherwise = NULL))
         ) %>%
  ...
  mutate(deltaOD_TimetoMax = map(.x = deltaOD_logistic_predict, possibly(~.$time[which.max(.$`.fitted`)], otherwise = NULL))  
         ) %>% 
  mutate(deltaOD_TimetoMax = as.numeric(unlist(deltaOD_TimetoMax))
         ) %>%
  ...
  mutate(deltaODtrunc_TimetoMax = as.numeric(unlist(deltaODtrunc_TimetoMax))
         ) %>%
   mutate(deltaODtrunc_exp = map(tubedata, possibly(~nlsLM(deltaOD ~ exp_eqn(x = time, Emu, Eintercept),
                            data = .x %>%
                            filter(deltaOD <= (max(deltaOD) * TruncFactor)),
                            start = list(Emu = (log(max(.$rollmeandeltaOD, na.rm = TRUE)) - log(min(.$rollmeandeltaOD, na.rm = TRUE)))/max(.$time), Eintercept = min(.$rollmeandeltaOD, na.rm = TRUE)),
                            control = list(maxiter = 500)), otherwise = NULL)),
         deltaODtrunc_exp_tidied =  map(deltaODtrunc_exp, possibly(tidy, otherwise = NULL)),
         deltaODtrunc_exp_param = map(deltaODtrunc_exp, possibly(glance, otherwise = NULL)),
         deltaODtrunc_exp_predict = map(deltaODtrunc_exp, possibly(augment, otherwise = NULL))
         ) %>%
